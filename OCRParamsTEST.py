...
def applyocr(params):
    ...
    isOnlyNumber = df['text'].str.strip().str.replace('.', '').str.isdigit()
...
def update(dummy=None):
    ...
    Scale = cv2.getTrackbarPos('Scale', 'Encontrados')/10
    DilateErodeKernelSize = cv2.getTrackbarPos('DilateErodeKernelSize', 'Encontrados')
    DilateIterations = cv2.getTrackbarPos('DilateIterations', 'Encontrados')
    ErodeIterations = cv2.getTrackbarPos('ErodeIterations', 'Encontrados')
    GaussianFilterKernel = cv2.getTrackbarPos('GaussianFilterKernel', 'Encontrados')
    AddWeighted1a = cv2.getTrackbarPos('AddWeighted 1a', 'Encontrados')/10
    AddWeighted2a = - cv2.getTrackbarPos('AddWeighted 2a', 'Encontrados')/10
...
imageRes = cv2.imread("segment/bar321L.png")
...
Scale = 1
DilateErodeKernelSize = 1
DilateIterations = 1
ErodeIterations = 1
GaussianFilterKernel = 1
AddWeighted1a = 1.8
AddWeighted2a = -0.6


cv2.imshow("Encontrados", imageRes)
cv2.createTrackbar('Scale', 'Encontrados', 1, 30, update)
cv2.createTrackbar('DilateErodeKernelSize', 'Encontrados', 1, 5, update)
cv2.createTrackbar('DilateIterations', 'Encontrados', 1, 10, update)
cv2.createTrackbar('ErodeIterations', 'Encontrados', 1, 10, update)
cv2.createTrackbar('GaussianFilterKernel', 'Encontrados', 1, 10, update)
cv2.createTrackbar('AddWeighted 1a', 'Encontrados', 18, 30, update)
cv2.createTrackbar('AddWeighted 2a', 'Encontrados', 6, 10, update)
